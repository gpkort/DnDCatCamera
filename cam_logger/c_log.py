...
class CameraLogger:
    ...
    def init_logger(logger_name:str = None,
        ...
        if CameraLogger.logger is None:
            CameraLogger.logger = logging.getLogger(name=os.path.join(log_dir, name)) 
            CameraLogger.logger.setLevel(logging_level)  
            handler = logging.FileHandler(os.path.join(log_dir, name))
            handler.setLevel(logging.DEBUG)       
            formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
            handler.setFormatter(formatter)
            CameraLogger.logger.addHandler(handler)               
        
            CameraLogger.clean_up_log_dir(log_dir, days)
        
    @staticmethod
    def is_initialized()->bool:
        return CameraLogger.logger is not None
    
    @staticmethod    
    def log_error(name:str, msg:str)->None:
        CameraLogger.logger.error(f'{name}: {msg}')
    
    @staticmethod    
    def log_info(name:str, msg:str)->None:
        CameraLogger.logger.info(f'{name}: {msg}') 
        
    @staticmethod
    def log_warning(name:str, msg:str)->None:
        CameraLogger.logger.warning(f'{name}: {msg}') 
        
    @staticmethod
    def log_debug(name:str, msg:str)->None:    
        CameraLogger.logger.debug(f'{name}: {msg}')
        
    @staticmethod
    def log_critical(name:str, msg:str)->None:    
        CameraLogger.logger.critical(f'{name}: {msg}')
        
    @staticmethod
    def log_exception(name:str, msg:str)->None:    
        CameraLogger.logger.exception(f'{name}: {msg}')
